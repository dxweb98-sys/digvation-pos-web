...
export function Dialog({
  ...
  return (
    <DialogRoot open={open} onClose={onClose} closeOnOverlay={closeOnOverlay} closeOnEscape={closeOnEscape} ariaLabel={ariaLabel ?? (typeof title === 'string' ? title : 'Dialog')} {...(ariaLabelledBy === undefined ? {} : { ariaLabelledBy })} {...(overlayClassName === undefined ? {} : { overlayClassName })} className={cn('flex max-h-[90vh] w-full flex-col rounded-t-2xl bg-[var(--color-surface)] shadow-xl transition-all duration-300 sm:max-h-[85vh] sm:rounded-xl', open ? 'translate-y-0 opacity-100 sm:scale-100' : 'translate-y-full opacity-0 sm:translate-y-0 sm:scale-95', sizeClasses[size], className)}>
